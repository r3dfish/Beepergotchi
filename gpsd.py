...
class gpsd_coord(plugins.Plugin):
    ...
    def on_ui_update(self, ui):
        ...
        if coords and all([
            ...
            altitude_in_feet = coords['Altitude'] * 3.28084
            ui.set("altitude", f" {altitude_in_feet:.0f} ft ")
            local_time = parser.parse(coords['Date']).astimezone()
            ui.set("date", local_time.strftime("%a %b %d").__str__())
            ui.set("time", local_time.strftime("%I:%M %p").__str__())
