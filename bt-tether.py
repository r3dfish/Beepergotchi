...
class BTTether(plugins.Plugin):
    ...
    def on_ui_update(self, ui):
        if (subprocess.run(['bluetoothctl', 'info'], capture_output=True, text=True)).stdout.find('Connected: yes') != -1:
            lines = subprocess.run(['ifconfig', 'bnep0'], capture_output=True, text=True).stdout
            if lines.find('inet') != -1:
                lines = lines.splitlines()
                for line in lines:
                    if "inet " in line:
                        self.status = line.split("inet ")[1].split()[0]
            else:
                self.status = 'Connected, no IP'
        else:
